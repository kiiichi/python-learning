...
oscilloscope.write(':ACQ:MDEP 10k')  # Set the acquisition memory depth to {AUTO|1k|10k|100k|1M|10M|25M|50M|100M|200M}
...
oscilloscope.write(':WAV:POIN 10000')  # Set number of points in the waveform record
time_per_sample = float(oscilloscope.query(':WAV:XINC?'))  # Get time per sample

waveform = oscilloscope.query(':WAV:DATA?')  # Send command to retrieve waveform data
waveform_data = waveform[11:]  # Remove header
...
with open('waveform.csv', 'w', newline='') as csvfile:
    ...
    for i, voltage in enumerate(waveform_data):
        time = i * time_per_sample  # Calculate time based on the sample index and time per sample
