...
function(event, _, subEvent, _, sourceGUID, _, _, _, targetGUID, _, _, _, spellID)
    ...
    if subEvent == "SPELL_CAST_SUCCESS" and aura_env.playerInCombat then
        ...
        if aura_env.lastCast > 0 and not aura_env.stunInLastCast and not isCasting and not isChanneling then
            delta = math.max(0, timestamp - aura_env.lastCast - FullGCD)
            if delta > 0 then
                aura_env.wasteGCDTime = aura_env.wasteGCDTime + delta
                aura_env.showTime = string.format("%.2f", aura_env.wasteGCDTime)
                if aura_env.battleDuration > 0 then
                    aura_env.showPercent = string.format("%.1f", (1 - aura_env.wasteGCDTime / aura_env.battleDuration) * 100)
                else
                    aura_env.showPercent = "100.0"
                end
            end
