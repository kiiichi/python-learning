...
function()
    ...
    local Cleave = function()
        ...
        if OffCooldown(ids.DireBeast) and ( IsPlayerSpell(ids.HuntmastersCallTalent) and GetPlayerStacks(ids.HuntmastersCallBuff) == 2 ) then
            NGSend("Dire Beast") return true end
        ...
        if OffCooldown(ids.CobraShot) and ( ((MaxFocus - CurrentFocus) / GetPowerRegen()) < WeakAuras.gcdDuration() * 2 or GetPlayerStacks(ids.HogstriderBuff) > 3 ) then
