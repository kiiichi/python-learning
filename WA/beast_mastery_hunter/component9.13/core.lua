env.test = function()
    ...
    local Cleave = function()
        ...
        if OffCooldown(ids.BestialWrath) and ( ( (GetRemainingAuraDuration("player", ids.HowlOfThePackLeaderCooldownBuff) - 12 < ( 12 / max(1.5/(1+0.01*UnitSpellHaste("player")), 0.75) * 0.5 )) and GetRemainingAuraDuration("player", ids.HowlOfThePackLeaderCooldownBuff) > 0 ) or SetPieces < 4 or IsPlayerSpell(ids.Multishot) )  then
