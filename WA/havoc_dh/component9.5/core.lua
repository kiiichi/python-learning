env.test = function()
    ...
    local Ar = function()
        ...
        -- Kichi update KTrigCD
        if OffCooldown(ids.ThrowGlaive) and FindSpellOverrideByID(ids.ThrowGlaive) == ids.ReaversGlaive and ( PlayerHasBuff(ids.GlaiveFlurryBuff) == false and PlayerHasBuff(ids.RendingStrikeBuff) == false and (GetRemainingAuraDuration("player", ids.ThrillOfTheFightDamageBuff) < 4 or GetRemainingSpellCooldown(ids.BladeDance) <= 2*FullGCD()) and ( PlayerHasBuff(ids.ThrillOfTheFightDamageBuff) or not ( CurrentTime - aura_env.LastDeathSweep < 1 ) or not Variables.RgInc ) and NearbyEnemies >= 2 and not TargetHasDebuff(ids.EssenceBreakDebuff) or FightRemains(60, 40) <= 10 ) then
            -- KTrig("Reavers Glaive") return true end
            if aura_env.config[tostring(ids.ThrowGlaive)] == true and aura_env.FlagKTrigCD then
                KTrigCD("Reavers Glaive")
            elseif aura_env.config[tostring(ids.ThrowGlaive)] ~= true then
