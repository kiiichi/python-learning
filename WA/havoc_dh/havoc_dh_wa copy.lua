...
function()
    ...
    local Ar = function()
        ...
        if OffCooldown(ids.ThrowGlaive) and FindSpellOverrideByID(ids.ThrowGlaive) == ids.ReaversGlaive and ( FightRemains(60, 40) < aura_env.config["SavingReaversGlaiveTime"] and not aura_env.config["SavingReaversGlaive"]==true or FightRemains(60, 40) >= aura_env.config["SavingReaversGlaiveTime"] ) and ( GetTargetStacks(ids.ReaversMarkDebuff)>=2 ) and ( PlayerHasBuff(ids.GlaiveFlurryBuff) == false and PlayerHasBuff(ids.RendingStrikeBuff) == false and GetRemainingAuraDuration("player", ids.ThrillOfTheFightDamageBuff) < max(1.5/(1+0.01*UnitSpellHaste("player")), 0.75) * 4 + ( (Variables.RgDs == 2) and 1 or 0 ) + (( GetRemainingSpellCooldown(ids.TheHunt) < max(1.5/(1+0.01*UnitSpellHaste("player")), 0.75) * 3 ) and 1 or 0) * 3 + (( GetRemainingSpellCooldown(ids.EyeBeam) < max(1.5/(1+0.01*UnitSpellHaste("player")), 0.75) * 3 and IsPlayerSpell(ids.ShatteredDestinyTalent) ) and 1 or 0) * 3 and ( Variables.RgDs == 0 or Variables.RgDs == 1 and OffCooldown(ids.BladeDance) or Variables.RgDs == 2 and GetRemainingSpellCooldown(ids.BladeDance) > 0 ) and ( PlayerHasBuff(ids.ThrillOfTheFightDamageBuff) or not ( CurrentTime - aura_env.LastDeathSweep < 1 ) or not Variables.RgInc ) and NearbyEnemies < 2 and CurrentTime - aura_env.ReaversGlaiveLastUsed > 5 and TargetHasDebuff(ids.EssenceBreakDebuff) == false and ( GetRemainingAuraDuration("player", ids.MetamorphosisBuff) > 2 or GetRemainingSpellCooldown(ids.EyeBeam) < 10 or FightRemains(60, 40) < 10 ) or FightRemains(60, 40) <= 10 ) then
            -- KTrig("Reavers Glaive") return true end
            if aura_env.config[tostring(ids.ThrowGlaive)] == true and aura_env.FlagKTrigCD then
                if FightRemains(60, 40) >= aura_env.config["SavingReaversGlaiveTime"] then KTrigCD("Reavers Glaive") end
                if FightRemains(60, 40) < aura_env.config["SavingReaversGlaiveTime"] then KTrigCD("Reavers Glaive", "Not Good") end
