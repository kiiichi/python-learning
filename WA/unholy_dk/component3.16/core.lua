env.test = function()
    ...
    Variables.EpidemicTargets = 3 + (IsPlayerSpell(ids.ImprovedDeathCoilTalent) and 1 or 0) + ( (IsPlayerSpell(ids.FrenziedBloodthirstTalent) and GetPlayerStacks(ids.EssenceOfTheBloodQueenBuff) > 5) and 1 or 0 ) + ( (IsPlayerSpell(ids.HungeringThirstTalent) and IsPlayerSpell(ids.HarbingerOfDoomTalent) and PlayerHasBuff(ids.SuddenDoomBuff)) and 1 or 0 )
    ...
    local AoeBurst = function()
        ...
        if OffCooldown(ids.DeathCoil) and ( FindSpellOverrideByID(ids.ScourgeStrike) ~= ids.VampiricStrike and NearbyEnemies < Variables.EpidemicTargets and ( not IsPlayerSpell(ids.BurstingSoresTalent) or IsPlayerSpell(ids.BurstingSoresTalent) and TargetsWithFesteringWounds < NearbyEnemies and TargetsWithFesteringWounds < NearbyEnemies * 0.4 and PlayerHasBuff(ids.SuddenDoomBuff) or PlayerHasBuff(ids.SuddenDoomBuff) and ( IsPlayerSpell(ids.DoomedBiddingTalent) and IsPlayerSpell(ids.MenacingMagusTalent) or IsPlayerSpell(ids.RottenTouchTalent) or GetRemainingDebuffDuration("target", ids.DeathRotDebuff) < WeakAuras.gcdDuration() ) ) ) then
            KTrig("Death Coil") return true end
        
        if OffCooldown(ids.Epidemic) and ( FindSpellOverrideByID(ids.ScourgeStrike) ~= ids.VampiricStrike and ( not IsPlayerSpell(ids.BurstingSoresTalent) or IsPlayerSpell(ids.BurstingSoresTalent) and TargetsWithFesteringWounds < NearbyEnemies and TargetsWithFesteringWounds < NearbyEnemies * 0.4 and PlayerHasBuff(ids.SuddenDoomBuff) or PlayerHasBuff(ids.SuddenDoomBuff) and ( PlayerHasBuff(ids.AFeastOfSoulsBuff) or GetRemainingDebuffDuration("target", ids.DeathRotDebuff) < WeakAuras.gcdDuration() or GetTargetStacks(ids.DeathRotDebuff) < 10 ) ) ) then
            KTrig("Epidemic") return true end
        
        if OffCooldown(ids.ScourgeStrike) and ( TargetHasDebuff(ids.ChainsOfIceTrollbaneSlowDebuff) ) then
            KTrig("Scourge Strike") return true end
        
        if OffCooldown(ids.ScourgeStrike) and ( GetTargetStacks(ids.FesteringWoundDebuff) >= 1 or FindSpellOverrideByID(ids.ScourgeStrike) == ids.VampiricStrike ) then
            KTrig("Scourge Strike") return true end
    ...
    local AoeSetup = function()
        ...
        if OffCooldown(ids.FesteringStrike) and ( GetRemainingSpellCooldown(ids.VileContagion) < 5 or TargetsWithFesteringWounds == NearbyEnemies and GetTargetStacks(ids.FesteringWoundDebuff) <= 4 ) then
            KTrig("Festering Strike") return true end
        
        if OffCooldown(ids.DeathCoil) and ( not Variables.PoolingRunicPower and PlayerHasBuff(ids.SuddenDoomBuff) and NearbyEnemies < Variables.EpidemicTargets ) then
            KTrig("Death Coil") return true end
        
        if OffCooldown(ids.Epidemic) and ( not Variables.PoolingRunicPower and PlayerHasBuff(ids.SuddenDoomBuff) ) then
            KTrig("Epidemic") return true end
        
        if OffCooldown(ids.FesteringStrike) and ( GetRemainingSpellCooldown(ids.Apocalypse) < WeakAuras.gcdDuration() and GetTargetStacks(ids.FesteringWoundDebuff) == 0 or TargetsWithFesteringWounds < NearbyEnemies ) then
            KTrig("Festering Strike") return true end
    ...
    local Cds = function()
        ...
        -- Kichi 3.16 change to NGA3.12 update
    ...
    local CdsAoe = function()
        if OffCooldown(ids.VileContagion) and ( GetTargetStacks(ids.FesteringWoundDebuff) >= 4 and ( GetRemainingSpellCooldown(ids.DeathAndDecay) < 3 or PlayerHasBuff(ids.DeathAndDecayBuff) and GetTargetStacks(ids.FesteringWoundDebuff) >= 4 ) or Variables.AddsRemain and GetTargetStacks(ids.FesteringWoundDebuff) == 6 ) then
    ...
    local SanSt = function()
        ...
        -- Kichi 3.3 for replace PlayerHasBuff to PetHasBuff, GetRemainingAuraDuration("player", ids.DarkTransformation) to GetRemainingAuraDuration("pet", ids.DarkTransformation)
        if OffCooldown(ids.ScourgeStrike) and ( PlayerHasBuff(ids.GiftOfTheSanlaynBuff) and FindSpellOverrideByID(ids.ScourgeStrike) == ids.VampiricStrike or IsPlayerSpell(ids.GiftOfTheSanlaynTalent) and PlayerHasBuff(ids.DarkTransformation) and GetRemainingAuraDuration("player", ids.DarkTransformation) < WeakAuras.gcdDuration() ) then
            KTrig("Scourge Strike") return true end
        ...
        if OffCooldown(ids.ScourgeStrike) and ( FindSpellOverrideByID(ids.ScourgeStrike) == ids.VampiricStrike and GetTargetStacks(ids.FesteringWoundDebuff) >= 1 ) then
            KTrig("Scourge Strike") return true end
