...
def nkt_read_power_mw(portname=PORTNAME_NKT):
    rdResult, power = registerReadU16(portname, 1, 0x17, -1)
    return power/100
    
def nkt_read_power_dbm(portname=PORTNAME_NKT):
    rdResult, power = registerReadS16(portname, 1, 0x90, -1)
    return power/100

def nkt_write_wl(wavelength, portname=PORTNAME_NKT):
